...
const mockPosts = [
  {
    ...
    content: "서버 컴포넌트에서 클라이언트 컴포넌트로 props를 넘길 때 직렬화 에러가 자꾸 뜨는데 도저히 이유를 모르겠네요.",
    author: "코딩초보",
    date: "방금 전",
    likes: 2,
    comments: 4,
  ...
  },
  {
    ...
    content: "클래스명이 너무 길어져서 가독성이 떨어질 때 'tailwind-merge'랑 'clsx' 조합해서 쓰면 진짜 편합니다.",
    author: "CSS장인",
    date: "10분 전",
    likes: 15,
    comments: 8,
  ...
  },
  {
    ...
    content: "이번에 취업 준비하면서 정리한 로드맵입니다. React, Next.js, TS 중심으로 정리했고 필요하신 분들 참고하세요!",
    author: "취준생",
    date: "1시간 전",
    likes: 42,
    comments: 12,
  ...
  },
  {
    ...
    content: "신입으로 지원하고 있는데 서류 통과율이 너무 낮네요... 포트폴리오를 갈아엎어야 할지 고민입니다.",
    author: "고민많음",
    date: "3시간 전",
    likes: 5,
    comments: 21,
        ...
        <CardHeader className="pb-3 flex flex-row justify-between items-start space-y-0">
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2">
              <Badge className="bg-red-500 hover:bg-red-600 border-none animate-pulse">LIVE</Badge>
          ...
          {isChatOpen && (
            <Button variant="ghost" size="icon" onClick={() => setIsChatOpen(false)} className="h-8 w-8 -mt-1 -mr-2">
                ...
                <div className="space-y-4 text-sm">
                  ...
                  <div className="flex gap-2 flex-row-reverse">
                    ...
                    <div className="text-right">
                        <span className="text-xs text-slate-500 block mb-1">나</span>
                        <div className="bg-slate-900 p-2 rounded-l-lg rounded-br-lg shadow-sm text-white">
                          네 안녕하세요! 어떤 게 궁금하신가요?
                        </div>
                    </div>
                  </div>
                    <p className="text-xs text-center text-slate-400 my-2">-- 실시간 대화 참여 중 --</p>
                </div>
              </div>
              
              <div className="flex gap-2">
                <Input placeholder="메시지를 입력하세요..." className="bg-white focus-visible:ring-slate-900" />
                <Button size="icon" className="bg-slate-900 hover:bg-slate-800 shrink-0">
